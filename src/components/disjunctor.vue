...
<template>
  <div class="disjunctor" :style="'height:'+Attrs.height+'px'">
    <button
      class="toggle"
      :style="'width:clamp(150px,'+useAttrs().width+'px, 100vw)'"
      @click="TOGGLE"
      aria-pressed="false"
        ...
        <span class="astrobear__container">
          <span class="astrobear">
            ...
            >
              ...
              <circle cx="158" cy="143" r="140" fill="url(#a)" fill-opacity="0.2" />
...
const TOGGLE = () => {
  const IS_PRESSED = document
    .querySelector("button")
    .matches("[aria-pressed=true]");

  document.body.setAttribute("data-dark-mode", IS_PRESSED ? false : true);
  emit("dark", IS_PRESSED ? false : true);
  document
    .querySelector("button")
    .setAttribute("aria-pressed", IS_PRESSED ? false : true);
};
const getData = () => {
  return DarkData;
};
